...
function insertEmot(e,forid){
	...
	if($('#'+fid)[0]){
		...
		var tipsEmotHTML = '<div class="'+tipsEmotid+'" id="'+tipsEmotid+'"><img id="'+tipsEmotid+'_src" src="'+window['Default_tplPath']+'images/emotion/24x24tranparent.gif" /><span class="alt" id="'+tipsEmotid+'_alt"></span/></div>';
...
function loadEmot(fid,forid){
	var strFace = '<div class="emot_inner replay_life" id="'+fid+'" style="display:none;"><div class="hde"><div class="tit">常用表情</div></div><a href="#" class="close">关闭</a><ul class="clearfix ul_0">';
	var i=0,k=0,len=emotData.length,path=window['Default_tplPath']+'images/emotion/',imgsrc='',sourceImgsrc='';
	for( ;i<len;i++){
...
function loadRevertReplay(o,parentid){
	...
	var sid = 'replay';
	if(!$('#'+sid)[0]){
		var strHTML = '<div class="write2014" id="replay" style="display:none;"><form id="myformReplay"><div class="cmt_txt" id="cmt_txtReplay" contenteditable="true"></div><div class="cmt_control clearfix"><div class="right">　<button type="submit" class="cmt_btn">提交</button></div><div class="left">文明上网 礼貌发帖</div></div></form></div>';
		$('body').append(strHTML);
...
function postNewRevert(o,node,activeid,styleid,shopid,tplid,chrmark,isrep,istg,parentid,score,score1){
	var btn_submit = $(o).find('button[type="submit"]');
...
$.fn.chackTextarea = function(a,c, e, b, callback) {
    ...
    t.submit(function(e){
		if($('#needLogin').val() === '1'){
			if($('#isLogin').val() === '0'){
				var url = nowdomain+'member/login.html?from='+encodeURIComponent(window.location);
		...
		var content = encodeURIComponent(removeHTMLTag(d.html()));
	...
	d.on('click',function(){
		...
		if (f.length > 0) {
			if (f.length > a) {
				...
				$("#" + e).html(parseInt(f.length/2) + '/100');
				$("#" + e).css({
					color: "#404040"
				});
